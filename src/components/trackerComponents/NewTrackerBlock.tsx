...
const NewTrackerBlock = () => {
  ...
  const isDisabled = allTrackersMeta.some(
  ...
  return (
    ...
    >
      <input
        className={`col-span-4 w-full px-2 py-0.5 border-2 bg-white ${isDisabled ? "border-red-300 focus:border-red-500" : "border-blue-100 focus:border-blue-300"} focus:outline-0 rounded-md text-xs`}
      ...
      <SubmitButton
        ...
        disabled={newTrackerId.trim() === "" || isDisabled}
      />
      {isDisabled && (
        <p className="ml-0.5 absolute top-7 lg:top-6 text-red-500 text-xs font-semibold col-span-5">
